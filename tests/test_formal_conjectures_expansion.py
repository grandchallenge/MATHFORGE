...
class FormalConjecturesExpansionTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.snapshot_ref_path = self.root / "snapshots" / "FC-GDM-002-ACTIVE-CAMPAIGN-EXPANSION.replay.json"
        self.snapshot_ref = json.loads(self.snapshot_ref_path.read_text(encoding="utf-8"))
        with zipfile.ZipFile(self.bundle) as archive:
            self.snapshot_bytes = archive.read(self.snapshot_ref["snapshot_member"]["name"])
        self.snapshot = json.loads(self.snapshot_bytes)
    ...
    def test_lock_and_snapshot_are_replay_derived_and_valid(self) -> None:
        ...
        self.assertEqual(self.validate(self.snapshot_ref, "formal_statement_replay_reference.schema.json"), [])
    ...
    def test_exact_replay_bundle_closes_over_manifest_and_committed_bytes(self) -> None:
        ...
        self.assertEqual(self.snapshot_ref["replay_bundle"]["git_blob_sha1"], "0c8ce12a81c8e40a3873a3b7ac73ee1232013f3f")
        self.assertEqual(hashlib.sha256(self.snapshot_bytes).hexdigest(), self.snapshot_ref["snapshot_member"]["sha256"])
        self.assertEqual(len(self.snapshot_bytes), self.snapshot_ref["snapshot_member"]["byte_length"])
        ...
        with zipfile.ZipFile(self.bundle) as archive:
            ...
            for record in manifest["records"]:
                ...
                if member == self.snapshot_ref["snapshot_member"]["name"]:
                    committed = self.snapshot_bytes
                elif member in {
